...
/**
 * Layer rules over the whole reactor (issue #110). engine-app's test classpath holds every module, so the
 * rules also catch dependencies the Maven module graph would allow (e.g. between two packages of engine-core).
 * Plain JUnit tests calling {@code check}, so no extra test engine is needed.
 */
class ArchitectureTest {
    ...
    void domainDoesNotDependOnConfigOrSpring() {
        noClasses()
                ...
                .should().dependOnClassesThat().resideInAnyPackage(
                        "dev.romeo.btctradingengine.config..",
                        "org.springframework..",
                        "jakarta..")
                .because("indicators, features and prediction are plain Java, reused as-is by the backtest")
                .check(classes);
    }
    ...
    void backtestDoesNotDependOnTrading() {
        noClasses()
                .that().resideInAPackage("dev.romeo.btctradingengine.backtest..")
                .should().dependOnClassesThat().resideInAPackage("dev.romeo.btctradingengine.trading..")
                .because("a backtest simulates its own positions and must never reach the order execution path")
                .check(classes);
    }
    ...
    void noCyclesBetweenTopLevelPackages() {
        slices()
                .matching("dev.romeo.btctradingengine.(*)..")
                .should().beFreeOfCycles()
                .ignoreDependency(resideInAPackage("dev.romeo.btctradingengine.dashboard.."),
