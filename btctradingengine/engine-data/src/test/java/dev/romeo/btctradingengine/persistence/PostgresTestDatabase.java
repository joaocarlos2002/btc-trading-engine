...
/**
 * One PostgreSQL container for every integration test of the module run (issue #103), started on
 * first use and migrated with the real Flyway migrations of {@code db/migration}, never a
 * hand-written schema. Tests call {@link #truncateAll()} to start from empty tables instead of
 * paying for a new container.
 *
 * <p>Only reached from {@code *IT} classes annotated {@code @Testcontainers(disabledWithoutDocker =
 * true)}, so without Docker they are skipped before this class starts anything.
 */
