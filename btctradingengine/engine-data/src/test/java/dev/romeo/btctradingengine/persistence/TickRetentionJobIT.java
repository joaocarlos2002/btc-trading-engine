...
/**
 * TickRetentionJob's batched DELETE ... WHERE id IN (SELECT ... LIMIT ?) on real PostgreSQL (issue #103): the
 * statement had only run against a fake connection, so its syntax and batching were never proven.
 */
...
class TickRetentionJobIT {
    ...
    private static TickRetentionJob job(int retentionDays, int batchSize) {
        return new TickRetentionJob(() -> PostgresTestDatabase.dataSource().getConnection(),
                Clock.fixed(NOW, ZoneOffset.UTC), retentionDays, batchSize, Duration.ofMinutes(60), 0);
    }
    ...
    void deletesOnlyTicksOlderThanRetentionInSeveralBatches() throws Exception {
        ...
        assertEquals(0, PostgresTestDatabase.count("SELECT count(*) FROM ticks WHERE time_ms < ?", cutoff.toEpochMilli()));
