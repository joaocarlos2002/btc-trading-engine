...
// Outline icons from Lucide (https://lucide.dev, ISC license), trimmed to the ones the app uses.
const ICONS = {
  activity:
...
/** <bte-icon name="activity" /> — decorative by default; pass a label to make it announce itself. */
@Component({
  selector: 'bte-icon',
  template: `<svg
    #svg
...
export class IconComponent {
  ...
  constructor() {
    // Static, trusted markup from the table above: set directly instead of through [innerHTML],
    // whose sanitizer would strip SVG elements.
    effect(() => {
      this.svg().nativeElement.innerHTML = ICONS[this.name()];
    });
  }
}
